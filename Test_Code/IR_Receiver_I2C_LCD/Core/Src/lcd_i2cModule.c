...
static  uint8_t chr_MAX = 16;

static  uint8_t Cursor_Data[2][16] = {
...
static uint8_t Cursor_Data[4][16] = {
  {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F}, //1. line DDRAM address
  {0x40,0x41,0x42,0x43,0x44,0x45,0x46,0x47,0x48,0x49,0x4A,0x4B,0x4C,0x4D,0x4E,0x4F}, //2. line DDRAM address
...
void LCD_i2cDeviceCheck(void)
{
	...
	HAL_Delay(50);
	hi2cx_define();
	while (HAL_I2C_IsDeviceReady(&hi2cx, 0X4E, 3, 1000) != HAL_OK)
	{	
...
void LCD_Set_Command(uint8_t cmd)
{
  ...
  data[3] = data_L;
	...
	hi2cx_define();	
	HAL_I2C_Master_Transmit(&hi2cx, 0X4E, (uint8_t*)data, 4, 200);
}
...
{	 
	...
	static uint8_t i[4] = {16,16,16,16}; //i follows the ch position while sliding.
